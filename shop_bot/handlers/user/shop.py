"""
هندلر فروشگاه
Shop handler — browse categories, products, and purchase flow
"""
...
async def show_product_detail(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if product.has_inventory:
        stock_text = f"\n📊 موجودی: {product.available_count} عدد"
    else:
        stock_text = ""
...
async def start_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async with get_db() as session:
        ...
        # ایجاد سفارش
        success, msg, order = await order_service.create_order(
        ...
        if method == "wallet":
            ...
            if success:
                ...
                if delivered and account:
                    await query.edit_message_text(
                        f"✅ پرداخت موفق!\n\n"
                        f"🎉 <b>اکانت شما:</b>\n"
                        f"<code>{account['account_text']}</code>",
                        parse_mode=ParseMode.HTML,
                    )
                else:
                    await query.edit_message_text(
            ...
            if ok:
                ...
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton("💳 پرداخت از طریق درگاه", url=url)
                ]])
                await query.edit_message_text(
            ...
            await query.edit_message_text(
                f"⚙️ درگاه «{method}» در حال پیاده‌سازی است.\nلطفاً روش دیگری انتخاب کنید."
            )
